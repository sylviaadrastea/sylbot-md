      ...
      case 'add':
        if (chat.welcome) {
          ...
          if (chat.remove) {
          	let groupMetadata = await this.groupMetadata(jid)
          for (let user of participants) {
            let pp = './src/avatar_contact 3.png'
            try {
              pp = await this.getProfilePicture(user)
            } catch (e) {
            } finally {
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'only owner , kamu tidak bisa menggunakan fitur ini',
    owner: 'only owner bot, kamu tidak bisa menggunakan fitur ini',
    mods: 'hanya moderator yang dapat menggunakan perintah ini',
    premium: 'kamu bukan user premium',
    group: 'gunakan fitur ini di grup',
    private: 'fitur ini hanya bisa di rc',
    admin: 'only admin , kamu bukan admin grup',
    botAdmin: 'botnya di jadiin admin dulu yyaa-!',
    unreg: 'daftar dulu kalo mau gunain fitur ini caranya ketik:\n\n*!daftar nama.umur*\n\nContoh: *!daftar ayangmu.16*'
  }[type]
